@extends('layouts.app')
@section('titulo', "Estudiantes - "."{$grupo->name_aula}")
@section('namePage', "Modulo: Grupo - Estudiantes - "."{$grupo->name_aula}")
@section('styles')
...
@section('content')
    <div class="card-body">
        <div class="row" id="ControlPanel">
            <div class="col-sm-6">
                <div class="mb-3">
                    <a href="{{route('grupos.index')}}"  class="btn btn-primary on-default ">Regresar <i class="fas fa-backward"></i></a>
                </div>
            </div>
        </div>
    ...
    <div id="inf" data-urltabla ="{{url()->current()}}"  data-url ="{{Config::get('app.url')}}"></div>
@endsection
@section('script')
    ...
    <script src="{{asset('js/tablas/estudiantesshow.js')}}"></script>
@endsection
